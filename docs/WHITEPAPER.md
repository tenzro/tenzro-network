...
- **Language** — Qwen 2 / 3 / 3.5 / 3.6 (dense and MoE), Gemma 3 / 4 (incl. Gemma 4 26B-A4B MoE), Mistral (incl. Mistral Small 3.2), Phi 3 / 4, DeepSeek V3 (native MTP) and V4 Pro / Flash, GLM 5 / 5.1 / 5.2 (5.2 with native MTP), Kimi K2 / K2.5 / K2.6 / K2.7-Code / K3, MiniMax M1 / M3, Granite, Granite-H, gpt-oss, Nemotron Nano. All run through the language runtime with full chat templates, streaming, and Anthropic-style SSE.
...
For training on sensitive data, the Confidential tier uses HPKE RFC 9180 base-mode wrapping of per-shard data keys to enclave-resident trainers. Each `SealedShardEnvelope` carries the ciphertext hash, wrapped data key, enclave pubkey, and enclave measurement; the manifest hash binds the full envelope set. At sign-up the syncer verifies the trainer's attestation report against a pinned vendor root, refuses simulated reports, and requires the report to commit to the enclave public key the sponsor wrapped that trainer's data key to and to carry the sponsor's expected measurement. Both expected values are read from the sealed manifest, so a trainer's own description of its enclave is never the evidence. Data unsealed only inside the trainer's TEE.
...
- **Default-deny RPC surface.** Every dispatched JSON-RPC method is classified as either admin-gated or open, and a method in neither is refused before its handler runs. A test reads the dispatcher's own source and fails while any arm is unclassified, so adding a method without deciding its gate makes it unreachable rather than public. This inverts an earlier allowlist whose default-open direction had already shipped thirteen unauthenticated methods. Privileged operations (cross-chain mint/burn, bridge authorization, compliance freeze, secure-mint policy mutation, delegation-scope tweaks, Canton operator surfaces) gate on the operator admin token; tenant API keys cannot escalate.
- **Operator-set service keys.** An operator may require a key before their node serves anyone, across JSON-RPC, MCP, A2A and the web API at once. Keys are stored as digests, compared in constant time, and never logged. The gate covers the service surface only: it has no representation for consensus or gossip, so a gated node structurally cannot stop validating, and liveness probes stay reachable so an orchestrator does not mistake a gated node for a dead one.
- **Confined interactive access.** A renter of a node's hardware reaches it over an ALPN on the node's existing QUIC endpoint — no inbound port, on the model of GCP's IAP and AWS's SSM. Three independent factors are required: a service key selecting the lease, a passkey ceremony against the renter's wallet establishing who they are, and the operator's authorized-wallet list deciding whether that wallet may. Sessions run inside a VM boundary or not at all; a node with no confinement backend refuses rather than degrading to a container namespace, which is not a boundary against someone holding a shell. A node advertising the TEE-provider role cannot rent out a shell on the same enclave, because interactive access invalidates what its measurement claims.
