...
| `tenzro-hybrid` default scheme: Ed25519 over `network‖asset‖maxAmountRequired‖payTo‖payer` | ✅ | `crates/tenzro-payments/src/x402/scheme.rs:280-336` |
...
| **Top-level `x402Version: 1` field** on PaymentRequirements per spec §3.1 | ✅ | `X402_WIRE_VERSION = 1` serialized as `x402Version` in `crates/tenzro-payments/src/x402/payment_required.rs` and `payment_payload.rs`. |
| **PaymentRequirements field naming** per spec §3.1: `network`, `maxAmountRequired`, `payTo`, `mimeType`, `description`, `resource`, `scheme`, `maxTimeoutSeconds`, `outputSchema` | ✅ | `X402PaymentRequirement` carries all spec fields in camelCase via `#[serde(rename_all = "camelCase")]` at `crates/tenzro-payments/src/x402/payment_required.rs`. |
| **PaymentPayload nested shape** per spec §4: `{x402Version, scheme, network, payload: {signature, authorization: {from, to, value, validAfter, validBefore, nonce}}}` | ✅ | `X402PaymentPayload { x402Version, scheme, network, payload: ExactSchemePayload { signature, authorization: ExactAuthorization { from, to, value, validAfter, validBefore, nonce } } }` at `crates/tenzro-payments/src/x402/payment_payload.rs`. |
| **`X-PAYMENT` request header** carrying base64(PaymentPayload-JSON) per spec §2.1 | ✅ | `X402PaymentServer` reads the `X-PAYMENT` header at `crates/tenzro-payments/src/x402/server.rs`; middleware path also wires base64 extraction. |
| **`X-PAYMENT-RESPONSE` response header** carrying base64(SettlementResponse-JSON) per spec §2.2 | ✅ | `X402PaymentServer::settle` stashes the canonical receipt body in `PaymentReceipt.extra["x_payment_response"]`; `payment_gate_handler` in `crates/tenzro-payments/src/middleware.rs` base64-encodes it and emits the `X-PAYMENT-RESPONSE` header on the forwarded response. |
