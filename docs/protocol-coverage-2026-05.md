...
**Tenzro implementation:** Native Tenzro precompiles at `0x101a` / `0x101b` / `0x101c` (per CLAUDE.md), Ethereum-mirror adapter at `crates/tenzro-identity/src/erc8004.rs`.
...
| Native EVM precompile selectors at `0x101a` (Identity), `0x101b` (Reputation), `0x101c` (Validation) — selector-byte-identical to ERC-8004 | ✅ | Confirmed by CLAUDE.md crate-details for `tenzro-vm`; `crates/tenzro-identity/src/erc8004.rs:106-117` is the source of truth |
| `AgentRecord { agent_id, agent_address, metadata_uri }` round-trip | ✅ | `crates/tenzro-identity/src/erc8004.rs:120-125, 281-300` |
| **`setAgentURI(bytes32,string)` selector** per ERC-8004 §IdentityRegistry | 🔴 | Not present in `selectors` mod at `crates/tenzro-identity/src/erc8004.rs:106-117`. Spec mandates URI rotation without re-registering. |
| **`setAgentWallet(bytes32,address)` selector** per ERC-8004 §IdentityRegistry | 🔴 | Not present. Spec lets the agent rotate its on-chain address. Belongs in `selectors` mod. |
| **`setMetadata(bytes32,bytes)` / `getMetadata(bytes32)`** for ERC-721-style key-value metadata per ERC-8004 §IdentityRegistry | 🔴 | Not present. |
| **`revokeFeedback(bytes32,bytes32)` selector** per ERC-8004 §ReputationRegistry | 🔴 | Not present. Spec §Reputation requires a revocation path so subjects can dispute. |
| **`appendResponse(bytes32,bytes32,string)` selector** per ERC-8004 §ReputationRegistry | 🔴 | Not present. Spec §Reputation lets the rated agent attach a public response. |
| **`getFeedback` / `getFeedbackCount` read selectors** per ERC-8004 §ReputationRegistry | ✅ | Adapter constants `selectors::GET_FEEDBACK = [0x7c,0x9d,0x4f,0x52]` and `selectors::GET_FEEDBACK_COUNT = [0x4e,0x71,0xa2,0x18]` byte-identical to precompile constants in `crates/tenzro-vm/src/evm/erc8004.rs:55-58`. Adapter wraps via `Erc8004Adapter::get_feedback` / `get_feedback_count` with full decoders for the precompile's wire shape (`(int8,string,bool)` for `getFeedback`, `uint256` for `getFeedbackCount`). |
| **`getValidation` read selector** per ERC-8004 §ValidationRegistry | ✅ | Adapter constant `selectors::GET_VALIDATION = [0x9b,0x2e,0x4f,0x33]` byte-identical to precompile constant in `crates/tenzro-vm/src/evm/erc8004.rs:64`. `Erc8004Adapter::get_validation` returns the full 6-field `DecodedValidation { subject, status, validator, task_uri, proof_uri, exists }` shape produced by the precompile. Test `decode_get_validation_round_trips_precompile_shape` round-trips the byte layout. |
| ERC-721 inheritance for IdentityRegistry (NFT representation of agents) | 🔴 | Not modeled — IdentityRegistry is precompile, not ERC-721. Native fast-path is acceptable for Tenzro but the Ethereum mirror loses ERC-721 transferability. Belongs in `crates/tenzro-vm/src/evm/erc8004.rs` (claimed to exist per pending audit). |
...
- **EVM precompile-level ERC-8004 selector validation** at `0x101a` / `0x101b` / `0x101c` — file path implied by CLAUDE.md but not directly read in this pass; verifying byte-for-byte selector parity with `crates/tenzro-identity/src/erc8004.rs::selectors` belongs in a follow-up.
