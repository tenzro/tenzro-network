...
- **ERC-8004 cross-chain reputation aggregation.** TIP-20 transfer events on Tempo where the agent DID is the sender get rolled into `submitFeedback` against the agent's registry-allocated `uint256 agentId` (resolved from the DID via `OnChainAgentRegistry::lookup_agent_id_by_did`) on the Tenzro `0x101b` precompile — same DID, multi-chain footprint.
...
1. **Tempo settlement adapter**: `TempoBridgeAdapter` + `TempoParticipant` + `Tip20Token` / `Tip20Balance` + `TempoConfig` are all live at `crates/tenzro-payments/src/tempo/{adapter,participant,stablecoin,config}.rs`. EIP-155 signing via k256 (`participant.rs:118`), RLP via `rlp` crate, Keccak-256 via `sha3`, JSON-RPC via `reqwest`. EIP-55 checksummed address formatting at `participant.rs:164`. Round-trip signing test at `participant.rs:990`.
2. **TIP-20 cross-VM enum + bridge byte-mapping**: `TokenVmType::TempoTip20` added to `crates/tenzro-token/src/cross_vm.rs:11`; `VmAddresses.tempo: Option<[u8; 20]>` (line 48) carries the TIP-20 contract address. `crates/tenzro-vm/src/cross_vm_bridge.rs` byte-mapping (`VM_TYPE_TEMPO_TIP20`) routes Tempo through the cross-VM dispatcher.
   - **Pending:** `TokenRegistry` (`crates/tenzro-token/src/registry.rs`) does not yet ingest TIP-20 contract addresses from `TempoConfig::stablecoin_addresses` — registry-side catalog mirror is the remaining wire-up.
3. **DID-anchored Tempo identity**: `SERVICE_TYPE_TEMPO_ACCOUNT = "TempoAccount"` in `crates/tenzro-identity/src/kya.rs` next to the MastercardKYA / VisaTAP constants; covered by `is_kya_service_type()`. Service entries persist via `IdentityRegistry::add_service_to_identity` (write-through to `CF_IDENTITIES`).
4. **MPP cart-mandate Tempo route** (Pending): `CheckoutMandate` (`crates/tenzro-payments/src/ap2/mod.rs:98`) carries no `accepted_chains` field yet. Adding the field — `accepted_chains: Vec<String>` with kebab-case chain selectors — lets the MPP router pick Tempo (`"tempo"`) per cart. The `principal_chain: "tempo"` audit trail and `MppReceipt.chain = "tempo"` defaults already hold; the gap is only the cart-side allow-list.
5. **`tenzro_tempoProtocolInfo` RPC**: handler at `crates/tenzro-node/src/rpc_integrations.rs::handle_tempo_protocol_info` (line 606) mirrors the shape of `handle_visa_tap_protocol_info` / `handle_x402_protocol_info`. Advertises Tempo chain ID (42431), Moderato/mainnet RPCs, configured TIP-20 contracts, EIP-155 signing flow, Simplex BFT finality model, and the DID-federation `service[].type = "TempoAccount"` extension.
6. **Cross-chain reputation aggregation** (later wave): TIP-20 transfer events on Tempo by an agent DID feed `submitFeedback` against the registry-allocated `uint256 agentId` (resolved via `OnChainAgentRegistry::lookup_agent_id_by_did`) on the Tenzro `0x101b` precompile — same DID, multi-chain footprint. Touches precompile dispatch; deferred behind the recognition surfaces above.
