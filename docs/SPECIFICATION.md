...
- **RPC surface.** `tenzro_registerStableAsset`, `tenzro_getStableAsset`, `tenzro_mintStableAsset`, `tenzro_redeemStableAsset` — all mirrored as Rust + Python MCP tools, Rust + TypeScript SDK methods, A2A skills, and `tenzro stable-asset` CLI subcommands.
