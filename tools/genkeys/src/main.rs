...
fn main() -> Result<()> {
    ...
    // crates/tenzro-node/src/config.rs (Genesis struct, version = 2).
    ...
    for i in 0..args.count {
        eprintln!(
            "       gcloud secrets versions add tenzro-validator-{i}-consensus \\\n         --data-file={0}/validator-{i}/consensus.seed --project=tenzro-infra",
            args.out_dir.display()
        );
        eprintln!(
            "       gcloud secrets versions add tenzro-validator-{i}-pq \\\n         --data-file={0}/validator-{i}/pq.seed --project=tenzro-infra",
            args.out_dir.display()
        );
        eprintln!(
            "       gcloud secrets versions add tenzro-validator-{i}-p2p \\\n         --data-file={0}/validator-{i}/p2p.seed --project=tenzro-infra",
            args.out_dir.display()
        );
    }
    eprintln!();
