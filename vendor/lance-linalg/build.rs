...
fn main() -> Result<(), String> {
    ...
    if target_arch == "aarch64" && target_os == "macos" {
        ...
        // Count kernels that actually compiled; see note below.
        let mut avx512_built = 0u8;
        ...
        // Only claim AVX-512 support when all three kernels built. The
        // Rust side gates its extern declarations on this one cfg, so a
        // partial success declares symbols that were never compiled.
        if avx512_built == 3 {
            println!("cargo:rustc-cfg=kernel_support=\"avx512\"");
        } else {
            println!(
                "cargo:warning=AVX-512 kernels incomplete ({}/3 built); using AVX2 path",
                avx512_built
            );
        }
