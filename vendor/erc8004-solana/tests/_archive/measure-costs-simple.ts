/**
 * Simple cost measurement using main wallet (no airdrops needed)
 */
...
describe("💰 Real Cost Measurement on Devnet", () => {
  ...
  after(async () => {
    ...
    for (const cost of costs) {
      ...
      console.log(`${cost.operation.padEnd(28)} | ${costSOL} | ${feeSOL} | ${cu}`);
    ...
    console.log("---------------------------- | -------------- | -------------- | -------------");
    console.log(`${"TOTAL".padEnd(28)} | ${(totalCost / 1e9).toFixed(9).padStart(14)} | ${(totalFee / 1e9).toFixed(9).padStart(14)} |`);
    ...
    fs.writeFileSync("/tmp/real-costs-devnet-final.json", JSON.stringify(report, null, 2));
