...
ggml_tensor * llama_model_kimi_k3::graph::res_mix(ggml_tensor * cur, ggml_tensor * score_w,
    ...
    // The convex combination is a reduction over ne1 of the stack. Expressed as
    // n_ckpt broadcast-multiplies against the individual checkpoints rather than
    // one fused ne1 reduction, because the fused form has no Metal kernel and
    // would split the graph onto the CPU. n_ckpt is bounded by
    // attn_res_block_size, so the unrolled form stays cheap.
    ggml_tensor * out = nullptr;
    for (int c = 0; c < n_ckpt; ++c) {
        ggml_tensor * p_c = ggml_cont(ctx0, ggml_view_2d(ctx0, probs, 1, n_tokens, probs->nb[1],
                                                        probs->nb[0] * c));
        ggml_tensor * term = ggml_mul(ctx0, ggml_reshape_2d(ctx0, ckpts[c], n_embd, n_tokens), p_c);
