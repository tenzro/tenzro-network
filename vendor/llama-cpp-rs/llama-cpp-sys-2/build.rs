...
fn main() {
    ...
    // The ggml RPC backend symbols (`ggml_backend_rpc_add_server`, …) are only
    // linked when the `rpc` feature compiled `GGML_RPC=ON`. Gate the
    // ggml-rpc.h include in wrapper.h on the same feature so the bindings only
    // reference symbols that are actually present. They already match the
    // `ggml_.*` allowlist, so no extra allowlist entry is needed.
    if cfg!(feature = "rpc") {
        bindings_builder = bindings_builder.clang_arg("-DLLAMA_RS_BUILD_RPC");
    }
    ...
    // The `rpc` feature builds the standalone `rpc-server` binary, which lives
    // under tools/rpc. tools/CMakeLists.txt only adds the rpc subdirectory when
    // GGML_RPC is on, and gates ui/cli/server behind LLAMA_BUILD_SERVER (kept
    // OFF), so turning tools on with RPC pulls in rpc-server without the heavy
    // server tooling.
    config.define(
        "LLAMA_BUILD_TOOLS",
        if cfg!(feature = "rpc") { "ON" } else { "OFF" },
    );
    if cfg!(feature = "rpc") {
    ...
    if matches!(target_os, TargetOs::Apple(_)) && !cfg!(feature = "blas") {
    ...
    if cfg!(feature = "sycl") {
        // Intel oneAPI SYCL. ggml-sycl only accepts GGML_SYCL_TARGET=INTEL and
        // requires the DPC++ compiler (icpx). Operators source setvars.sh from
        // the oneAPI toolkit before building; honour an explicit compiler if the
        // environment already set one, otherwise default to icx/icpx.
        config.define("GGML_SYCL", "ON");
        config.define("GGML_SYCL_TARGET", "INTEL");
        if env::var("CMAKE_C_COMPILER").is_err() {
            config.define("CMAKE_C_COMPILER", "icx");
        }
        if env::var("CMAKE_CXX_COMPILER").is_err() {
            config.define(
                "CMAKE_CXX_COMPILER",
    ...
    if cfg!(feature = "openvino") {
        // OpenVINO GGUF backend (Intel CPU / GPU / NPU). CMake locates the
        // runtime via OpenVINO_DIR, which `source setupvars.sh` exports; the
        // runtime device is chosen at inference time via GGML_OPENVINO_DEVICE.
    ...
    // Locate the `rpc-server` binary cmake just built and export its path so
    // dependents (the cluster serving runtime) can spawn it. cmake places tool
    // binaries under <build>/bin on most generators; fall back to the rpc tool
    // dir for single-config generators that keep binaries beside their target.
    if cfg!(feature = "rpc") {
        let exe = if matches!(target_os, TargetOs::Windows(_)) {
            "rpc-server.exe"
        } else {
            "rpc-server"
        };
        let candidates = [
            build_dir.join("bin").join(exe),
            build_dir.join("build").join("bin").join(exe),
            build_dir.join("build").join("tools").join("rpc").join(exe),
        ];
