...
pub enum LlamaCppError {
    ...
    /// Failed to register a remote ggml RPC endpoint as a backend device.
    #[cfg(feature = "rpc")]
    #[error("failed to add ggml RPC server endpoint: {0}")]
    BackendRpcAddServerFailed(String),
...
/// Path to the `rpc-server` binary built alongside this crate, if the `rpc`
/// feature was enabled at build time. The binary lets a remote machine act as
/// a ggml device executor over the RPC backend. It is insecure on open
/// networks and must be tunneled behind an authenticated transport.
///
/// Returns `None` when the crate was built without the `rpc` feature.
#[must_use]
pub fn rpc_server_bin() -> Option<&'static str> {
    option_env!("LLAMA_RPC_SERVER_BIN")
}

/// Register a remote ggml RPC endpoint as a backend device.
///
/// `endpoint` is a `host:port` string for a reachable `rpc-server`. After this
/// returns successfully the endpoint's device(s) appear in the global backend
/// registry and can be selected for inference like any local device. Call once
/// per endpoint, before loading the model that should span the cluster.
///
/// The ggml RPC backend trusts its peer completely and performs no
/// authentication, so `endpoint` must be a loopback address that is spliced to
/// the real peer through an authenticated tunnel — never a raw LAN/WAN address.
///
/// Only available when the crate is built with the `rpc` feature.
///
/// # Errors
/// Returns [`LlamaCppError::BackendRpcAddServerFailed`] if llama.cpp could not
/// register the endpoint (unreachable socket, protocol-version mismatch).
#[cfg(feature = "rpc")]
pub fn rpc_add_server(endpoint: &str) -> Result<()> {
    let c_endpoint = std::ffi::CString::new(endpoint)
        .map_err(|_| LlamaCppError::BackendRpcAddServerFailed(endpoint.to_string()))?;
    // SAFETY: `ggml_backend_rpc_add_server` copies the endpoint string; the
    // CString stays alive for the call. A null return means registration
    // failed (the backend reg was not produced).
    let reg = unsafe { llama_cpp_sys_2::ggml_backend_rpc_add_server(c_endpoint.as_ptr()) };
    if reg.is_null() {
        return Err(LlamaCppError::BackendRpcAddServerFailed(endpoint.to_string()));
    }
    Ok(())
}

/// Query the free and total device memory (in bytes) reported by a remote ggml
/// RPC endpoint. Used to validate that a member can hold its assigned layer
/// range before the model is loaded.
///
/// Only available when the crate is built with the `rpc` feature.
///
/// # Errors
/// Returns [`LlamaCppError::BackendRpcAddServerFailed`] if the endpoint string
/// contains an interior nul byte.
#[cfg(feature = "rpc")]
pub fn rpc_device_memory(endpoint: &str, device: u32) -> Result<(usize, usize)> {
    let c_endpoint = std::ffi::CString::new(endpoint)
        .map_err(|_| LlamaCppError::BackendRpcAddServerFailed(endpoint.to_string()))?;
