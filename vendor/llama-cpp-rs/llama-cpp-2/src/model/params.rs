...
impl LlamaModelParams {
    ...
    /// Sets an explicit `tensor_split`: the proportion of the model placed on
    /// each device, indexed to match [`Self::with_devices`]. Values are
    /// relative weights (llama.cpp normalizes them), so passing per-stage layer
    /// counts reproduces a layer-range pipeline assignment exactly.
    ///
    /// The backing buffer is owned by `self`, so the raw pointer handed to
    /// llama.cpp stays valid for the lifetime of these params.
    #[must_use]
