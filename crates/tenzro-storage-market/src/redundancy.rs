...
impl RedundancyScheme {
    ...
    pub fn erasure(data: usize, parity: usize) -> Result<Self> {
        if data == 0 {
            return Err(StorageMarketError::InvalidRedundancy(
...
pub fn encode(data: &[u8], scheme: RedundancyScheme) -> Result<Vec<Shard>> {
    ...
    let rs = ReedSolomon::new(k, m).map_err(|e| StorageMarketError::Erasure(e.to_string()))?;
    ...
    rs.encode(&mut shards)
        .map_err(|e| StorageMarketError::Erasure(e.to_string()))?;
...
/// truncated to `original_len`. Otherwise returns [`StorageMarketError::InsufficientShards`].
pub fn reconstruct(
    present: &[Shard],
    ...
    for slot in slots.iter().take(k) {
        let bytes = slot.as_ref().ok_or_else(|| StorageMarketError::Erasure(
...
mod tests {
    ...
    fn fails_past_m_losses() {
        ...
        assert!(matches!(err, StorageMarketError::InsufficientShards { have: 3, need: 4, .. }));
    }
