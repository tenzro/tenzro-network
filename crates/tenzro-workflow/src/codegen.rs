...
impl WorkflowDamlCodegen {
    ...
    ///   known parties).
    pub fn emit(workflow: &Workflow, map: &DamlMap, obligations: &[Obligation]) -> crate::Result<String> {
        ...
        let _ = writeln!(s, "-- Source workflow: 0x{}", hex::encode(workflow.workflow_id.as_bytes()));
        let _ = writeln!(s, "-- Canonical hash:  0x{}", hex::encode(workflow.canonical_hash().as_bytes()));
        let _ = writeln!(s, "-- DamlMap module:  {}", map.module);
        let _ = writeln!(s, "-- DO NOT EDIT — re-run codegen if the workflow spec changes.");
        let _ = writeln!(s);
        ...
        let _ = writeln!(s, "data {} = {} with", map.obligation_spec_type, map.obligation_spec_type);
        ...
        let _ = writeln!(s);
        let _ = writeln!(s, "    choice Sign : Either (ContractId {}) (ContractId {})", pending, active);
        let _ = writeln!(s, "      with");
        let _ = writeln!(s, "        signer : Party");
        let _ = writeln!(s, "      controller signer");
        let _ = writeln!(s, "      do");
        let _ = writeln!(s, "        assertMsg \"signer not a participant\" (signer `elem` participants)");
        let _ = writeln!(s, "        assertMsg \"already signed\" (signer `notElem` signatories_so_far)");
        ...
        let _ = writeln!(s, "            cid <- create this with signatories_so_far = next");
        ...
        if let Some(iface) = &map.external_interface {
            let _ = writeln!(s);
            let _ = writeln!(s, "    -- External interface binding (e.g. CIP-56 TransferInstruction)");
        ...
        for ob in obligations {
            ...
            let _ = writeln!(s, "      controller obligor_party_for \"0x{}\" obligations participants", hex::encode(ob.obligation_id.as_bytes()));
            let _ = writeln!(s, "      do");
            let _ = writeln!(s, "        let next_discharged = \"0x{}\" :: discharged_ids", hex::encode(ob.obligation_id.as_bytes()));
            let _ = writeln!(s, "        create this with discharged_ids = next_discharged");
        }

        // Helper used by every discharge choice to resolve obligor → Party.
        let _ = writeln!(s);
        let _ = writeln!(s, "obligor_party_for : Text -> [{}] -> [Party] -> Party", map.obligation_spec_type);
        let _ = writeln!(s, "obligor_party_for oid obs _ =");
        let _ = writeln!(s, "  let matches = [o.obligor | o <- obs, o.obligation_id == oid]");
        let _ = writeln!(s, "  in fromOptional (error (\"unknown obligation \" <> oid))");
        let _ = writeln!(s, "       (if length matches > 0 then Some (matches !! 0) else None)");
        let _ = writeln!(s);
        ...
        if !auditors.is_empty() {
            let _ = writeln!(s, "-- Auditor parties (observer-only via party allocation):");
            for did in auditors {
                let _ = writeln!(s, "--   {}", did);
            }
            let _ = writeln!(s);
...
fn discharge_kind_label(k: &DischargeProofKind) -> String {
    match k {
        ...
        DischargeProofKind::CantonExercise { template_id, choice } => {
...
mod tests {
    use super::*;
    use crate::obligation::{AssetRef, Obligation, ObligationKind, ObligationStatus, DischargeProofKind};
    ...
    fn mk_obligation(workflow_id: Hash, obligor: &str, obligee: &str, nonce: u64, kind: ObligationKind, dp: DischargeProofKind) -> Obligation {
        Obligation {
        ...
        DamlMap {
            ...
            obligation_choices: vec![
                ObligationChoiceMap {
                    ...
                    choice_args: vec![
                        DamlArgField { name: "amount".into(), daml_type: "Decimal".into() },
                        DamlArgField { name: "asset_symbol".into(), daml_type: "Text".into() },
                        DamlArgField { name: "payment_receipt_hash".into(), daml_type: "Text".into() },
                    ],
                },
                ObligationChoiceMap {
                    ...
                    choice_args: vec![
                        DamlArgField { name: "resource_did".into(), daml_type: "Text".into() },
                        DamlArgField { name: "qty".into(), daml_type: "Int".into() },
                        DamlArgField { name: "delivery_credential_hash".into(), daml_type: "Text".into() },
                    ],
                },
            ],
    ...
    fn emit_rejects_unknown_obligation_tag() {
        ...
        let ob = mk_obligation(
            ...
            0,
            ObligationKind::Settle { settlement_id: Hash::zero() },
    ...
    fn emit_rejects_obligor_not_in_participants() {
        ...
        let ob = mk_obligation(
            ...
            "did:tenzro:human:bob:1",
            0,
            ObligationKind::Pay { amount_wei: 1_000, asset: AssetRef { chain: "tenzro".into(), symbol: "USDC".into(), token_address: None } },
    ...
    fn reference_daml_map_parses() {
        ...
        let raw = include_str!(
            "../reference_workflows/autonomous_procurement_daml_map.json"
        );
    ...
    fn emit_contains_required_skeleton() {
        ...
        let ob2 = mk_obligation(
            ...
            "did:tenzro:machine:alice:bot:1",
            1,
            ObligationKind::Deliver { resource_did: "did:tenzro:resource:widget:1".into(), qty: 5 },
