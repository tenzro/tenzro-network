        ...
        match (
            ...
            (false, false) => {
                tenzro_crypto::pq::ml_dsa_verify(
                    &credential.pq_public_key,
                    message.as_bytes(),
                    &credential.pq_signature,
                )
                .map_err(|e| {
                    PaymentError::CryptoError(format!("PQ signature verification failed: {}", e))
                })?;
            }
            _ => {
                return Err(PaymentError::CredentialError(
                    "Credential post-quantum leg is incomplete: pq_signature and pq_public_key must both be present or both absent"
                        .to_string(),
                ));
            }
        }
...
impl PaymentProtocol for VisaTapServer {
    ...
    async fn create_credential(
        ...
        // Bind the credential post-quantum with the same Tenzro-hybrid
        // (Ed25519 + ML-DSA-65) construction the x402 and MPP backends use,
        // over the shared credential preimage. This is a Tenzro-native
        // envelope on `PaymentCredential.pq_signature`/`pq_public_key`,
        // independent of the RFC 9421 HTTP-signature leg below.
        let pq_key = tenzro_crypto::pq::MlDsaSigningKey::generate();
        let pq_public_key = pq_key.verifying_key_bytes().to_vec();
        let pq_signature = pq_key.sign(message.as_bytes());

        // Visa TAP authenticates the HTTP request itself via an RFC 9421 HTTP
        // Message Signature carried in `extra` (signature_input, signature).
        // That leg stays pure-standard Ed25519 for relying-party interop —
        // RFC 9421 defines no hybrid signing algorithm for HTTP message
        // signatures, so the post-quantum binding lives only on the
        // Tenzro-native credential fields above, never on the wire signature.
...
mod tests {
    ...
    async fn test_create_credential() {
        ...
        // The credential carries a populated Tenzro-native post-quantum leg
        // (ML-DSA-65) alongside the classical Ed25519 signature, and both
        // round-trip through the credential-signature verifier.
        assert!(!credential.pq_signature.is_empty());
        assert!(!credential.pq_public_key.is_empty());
        VisaTapServer::verify_credential_signature(&challenge, &credential).unwrap();

        // A tampered post-quantum leg is rejected.
        let mut tampered = credential.clone();
        tampered.pq_signature[0] ^= 0xff;
        assert!(VisaTapServer::verify_credential_signature(&challenge, &tampered).is_err());

        // An incomplete post-quantum leg (public key present, signature absent)
        // is rejected.
        let mut incomplete = credential.clone();
        incomplete.pq_signature.clear();
        assert!(VisaTapServer::verify_credential_signature(&challenge, &incomplete).is_err());
    }
