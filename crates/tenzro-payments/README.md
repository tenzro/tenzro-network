...
- **AP2 v0.2 (Agent Payments Protocol)** — Google/FIDO-backed protocol for verifiable agent payments using Verifiable Digital Credentials (VDC) for Checkout and Payment mandates. Five RPC surfaces: `tenzro_ap2SignMandate` (Ed25519 signing of the canonical preimage by the wallet bound to `signer_did`), `tenzro_ap2VerifyMandate`, `tenzro_ap2ValidateMandatePair` (enforces five nested ceilings: AP2 CheckoutMandate, TDIP `DelegationScope`, runtime `SpendingPolicy`, on-chain escrow, Stripe SPT usage limits), `tenzro_listMandates` (persisted checkout/payment pairs by controller DID), and `tenzro_ap2ProtocolInfo`
...
- **Stripe SPT (SharedPaymentToken)** — Token primitive that pairs with the MPP wire and Tempo settlement layers. `tenzro_sptIssue` signs an SPT bound to a principal/agent DID pair after `SptCeilingResolver` cross-checks the requested cap against the principal's `DelegationScope` and runtime `SpendingPolicy`. `tenzro_sptVerify` checks the SPT signature, principal/agent DID activity, and remaining cap. AP2 mandate-pair validation cross-checks `usage_limits ≥ total_amount`. ERC-8004 `ReputationRegistry` cross-write on every settled outcome. `granted_token.deactivated` webhook cascades into TDIP `apply_remote_revocation`.
- **ERC-8004 (Jan 2026 revision) Trustless Agents Registry** — full surface across IdentityRegistry, ReputationRegistry, and ValidationRegistry. `agentId` is a sequential `uint256` (1-indexed) allocated by the registry at `register*()` time — server-allocated, never derivable client-side. The TDIP `IdentityData::Machine.erc8004_agent_id` field captures the allocation; reverse DID → id lookup via `OnChainAgentRegistry::lookup_agent_id_by_did`. Calldata is byte-identical against either the native Tenzro precompiles (`0x101a` / `0x101b` / `0x101c`) or an Ethereum mirror.
- **x402 v1** — Coinbase's HTTP 402 payment protocol with facilitator-based settlement
- **Local x402 facilitator (default)** — `LocalFacilitatorVerifier` runs the exact/EVM scheme's eight admission checks and settles the `transferWithAuthorization` transaction through the operator's own `tenzro_bridge::EvmTransactionSigner`, without routing the signed authorization to a remote facilitator HTTP endpoint. This is the default settlement route: the operator brokers the transaction through a signer it controls. The Coinbase CDP facilitator remains constructible as an alternative when an operator prefers to defer to it
...
- **MandateKind** — `Checkout` | `Payment`; carries the AP2 v0.2 `vct` claim string, with an `open` variant for the human-not-present flow
- **CheckoutMandate** — The principal's pre-authorization envelope: principal/agent DIDs, `max_amount`, asset, `expires_at`, optional merchant/category/chain allow-lists, `max_uses`, and the Tenzro extensions `escrow_id`, `agent_bond_id`, `spt_grant_id`
- **PaymentMandate** — The agent's committed purchase: `checkout_mandate_id` plus a SHA-256 `checkout_hash` binding it to a specific parent, line items, `total_amount`, asset, and settlement `chain`
- **CartItem** — One line item on a PaymentMandate (sku, description, quantity, unit price, total, optional category)
- **PresenceMode** — `HumanPresent` (principal confirms interactively) | `HumanNotPresent` (agent commits within pre-delegated scope)
- **MandateCnf** — AP2 v0.2 SD-JWT VC `cnf` confirmation claim: an inline JWK per RFC 7800, or the Tenzro extension form naming a DID whose DID Document holds the binding key
- **MandatePayload** — Tagged union of the two mandate kinds; the body of a `Vdc`
- **Vdc** — Verifiable Digital Credential envelope: version, kind, payload, signer DID, signer Ed25519 public key, signature over the canonical preimage, and algorithm identifier
- **MandateValidator** — Verifies a single VDC signature and cross-validates a PaymentMandate against its parent CheckoutMandate (checkout hash, expiry, principal/agent binding, merchant and chain allow-lists, amount bounds), with variants that additionally enforce the TDIP `DelegationScope` and the runtime `SpendingPolicy`
