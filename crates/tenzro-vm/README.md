...
- **Secure-Mint Registry** (`secure_mint.rs`): per-token 1:1 reserve-attestation invariant for tokenized assets, keyed by the 20-byte token address. `SecureMintPolicy { asset_id, reserve, circulating, por_feed_id, attester_did, attestation_hash, attested_at, ttl_secs, heartbeat_secs, mint_window_cap, mint_window_secs, paused }` (`asset_id` is the CAIP-19 reserve asset, not the key); `check_and_mint` is fail-closed and gates in order: per-token/global pause → attestation freshness (`ttl_secs`) → PoR feed liveness (`heartbeat_secs`) → rolling velocity cap (`mint_window_cap`/`mint_window_secs`) → reserve floor (`circulating + amount ≤ reserve`). `record_burn` is bounded (rejects `amount > circulating`). Per-token and global issuance circuit breakers (`set_paused` / `set_global_pause`). `TokenizedEquityProfile` sidecar for tokenized-equity-class assets (CCT pool address, ISIN, CUSIP, per-share ratio, corporate-action hash). Precompile slot reserved at `0x0000…00001024`
