...
- **`TimeseriesRuntime`** — `ForecastModel` trait + `GenericForecast` ORT implementation. Supports `[1, context_len] -> [1, horizon]` and quantile output `[1, horizon, n_quantiles]`. Powers Chronos-2, Chronos-Bolt small/base, TimesFM 2.5 200M, Granite-TTM-r2.
...
- **`SegmentationRuntime`** — two-pass encoder/decoder runtime. Encoder caches per-image embedding; decoder takes embedding + prompts (points/boxes) → masks. API: `segment(model_id, image_bytes, Vec<SegmentPrompt>)`. Catalog: SAM 3 / 3.1, SAM 2 base/large, EdgeSAM, MobileSAM.
- **`DetectionRuntime`** — NMS-free DETR-family postprocessing (sigmoid + score threshold). API: `detect(model_id, image_bytes, score_threshold) -> Vec<Detection>` with `{bbox, label_id, score}`. Catalog: RF-DETR n/s/m/b/l/2xl, D-FINE n/s/m/l/x.
- **`AudioRuntime`** — ASR-only for wave 1. WAV decode via `hound`, MP3/FLAC via `symphonia`, mel-spectrogram via `realfft`. Supports Whisper-style encoder/decoder bundles, Moonshine v2 single-encoder, and NVIDIA Parakeet-TDT / Canary triple-bundles. Catalog: Moonshine v2 tiny/base, Distil-Whisper small.en/medium.en/large-v3, Whisper-large-v3-turbo, Parakeet-TDT-0.6B-v3, Canary-1B-Flash.
...
- **Timeseries forecast** — `OnnxForecastEntry` + `get_forecast_catalog()`: Chronos-2, Chronos-Bolt small/base, TimesFM 2.5 200M, Granite-TTM-r2.
...
- **Segmentation** — `OnnxSegmentationEntry` + `get_segmentation_catalog()`: SAM 3 / 3.1, SAM 2 base/large, EdgeSAM, MobileSAM.
