...
**Quality tiers.** `quality_tier()` sorts a candidate into `Strong` or `Cheap` on two facts only: it is `Strong` at or above a 30B parameter count, or at or above 7B while carrying a `code` or `reasoning` capability tag. Deliberately absent is context length — used as an independent term it classified every model shipping today as `Strong`, which inverted the ladder, because the cost term then picked the smallest candidate for every rung. Both inputs are catalog-owned: `capabilities_for_family()` states the tag claim per family, `to_model_info()` stamps it on a locally served model, `refresh_catalog_fields()` re-derives it on a stored row, and `network_catalog` recovers it on a gossiped offer. A model between the two floors reaches the strong tier by its tag alone, so a candidate that loses those tags anywhere along that path silently drops out of every `quality_floor = strong` request — and surfaces to the caller as an unsatisfiable budget rather than a missing tag.
...
- **Multi-Token Prediction (MTP) speculative decoding** — the runtime keeps a `loaded_drafters` map alongside `loaded_models`; when a drafter is loaded for a target and the request carries `draft_n`, generation runs real speculative decoding: the drafter proposes a block of candidate tokens, the target verifies them in one batched decode, and the accepted prefix advances the stream. Candidates come from one of two sources: a small **paired drafter** checkpoint auto-loaded from the catalog's `drafter_id` (downloaded in the background if absent), or a **native embedded MTP head** that self-drafts against the target's own hidden state (`drafter_id` unset, `mtp_kind: DraftMtp` — e.g. `qwen3.8-27b`), which needs no second checkpoint. A drafter problem never fails the serve. `ModelError::MtpUnavailable` is returned only when `draft_n` is requested with no drafter loaded for that target.
...
Admission differs from the in-process path and the difference is deliberate. A local GGUF decodes behind a single mutex, so a model carrying an mmproj projector or an MTP drafter is admitted at one request at a time; an external engine batches continuously on its own, so it is admitted at the node's whole slot budget (`max_slots()`) and anything past that is the upstream scheduler's to queue. Applying the serial cap here would throttle precisely the models worth fronting with vLLM. Both the serve path and the restore-at-boot path register this, because a model with no load-tracker entry reports a limit of zero and has every request shed as `at-capacity` while the engine behind it sits idle.
