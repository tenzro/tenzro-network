...
- **Intent Routing**: routeIntent, recordRouteOutcome, routeDifficultyStats, getRouterMetrics — a caller states what it wants (use case, budget, quality floor, token estimates) and the `MetaRouter` picks the model. Candidates come from both the operator's own catalog and `network_catalog`, the gossiped set of signed offers other providers are serving, so the winning offer names its own payee and the settlement split follows from it. An announcement is smaller than the model it describes — it carries one category tag — so a gossiped offer has its capability tags recovered from the local catalog by model id before it is tiered, the same claim a locally served model gets. Everything the provider is actually asserting (price, endpoint, schedule, liveness) is taken from the announcement and never from the catalog; a model the catalog does not know keeps exactly what was announced. `routeIntent` clusters the prompt and scores each candidate against that cluster's observed outcome history, giving cold models an optimism bonus so they are tried rather than starved; `recordRouteOutcome` feeds the result back
