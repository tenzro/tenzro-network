...
use tenzro_payments::x402::{
    X402Facilitator, X402PaymentPayload, X402PaymentRequired,
};
...
async fn x402_pay_resource() {
    ...
    // Build a payment requirement valid for 5 minutes. Default scheme is
    // `tenzro-hybrid`, which requires an Ed25519 signature over the
    ...
    // Build a real Ed25519-signed payload. The facilitator now dispatches
    // verification through SchemeRegistry, so a structurally-valid payload
    // alone is no longer enough — the signature must verify under the
    // selected scheme.
    ...
    // A payload on an unsupported chain must be rejected.
    let wrong_chain =
        X402PaymentPayload::new("ethereum", "USDC", "1000", &payer_hex, "auth-blob");
    let rejected = facilitator
        .verify(&requirements, &wrong_chain)
        .await
        .expect("verify");
    assert!(!rejected, "x402 payload on unsupported chain must be rejected");

    // A payload missing the signature must now be rejected — the previous
    // "skip verification on empty signature" shortcut is gone.
