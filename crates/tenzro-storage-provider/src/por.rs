...
pub async fn verify_challenge(
    ...
    if response.object_id != challenge.object_id {
        return Err(StorageProviderError::ChallengeFailed {
            object_id: challenge.object_id.clone(),
    ...
    for (pos, &idx) in challenge.shard_indices.iter().enumerate() {
        let sref = descriptor
            ...
            .iter()
            .find(|s| s.index == idx)
            .ok_or_else(|| StorageProviderError::ShardNotFound {
                object_id: challenge.object_id.clone(),
                shard_index: idx,
            })?;
...
mod tests {
    ...
    async fn stored() -> (
        ...
        let desc = provider
            .store_object(
                ...
                &data,
                scheme,
                tenzro_types::access_policy::AccessPolicy::owner_only("did:tenzro:human:por"),
                None,
            )
            .await
            .unwrap();
    ...
    async fn lying_provider_fails_challenge() {
        ...
        let err = verify_challenge(&desc, &challenge, &response, &resolver)
            .await
            .unwrap_err();
        assert!(matches!(err, StorageProviderError::ChallengeFailed { .. }));
    }
    ...
    fn challenge_samples_distinct_indices_within_bounds() {
        let desc = ObjectDescriptor {
            ...
            shards: (0..6)
                ...
                .collect(),
            access_policy: tenzro_types::access_policy::AccessPolicy::owner_only(
                "did:tenzro:human:por",
            ),
            confidential: None,
        };
