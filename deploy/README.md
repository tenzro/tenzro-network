...
**GCP Project:** `tenzro-operator-project`
**GKE Cluster (example):** `tenzro-testnet` in `us-central1-a`
**Container Registry:** `us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node`
...
**Internal Services:**
- JSON-RPC (8545) - Proxied via `rpc.tenzro.xyz`
- Web API (8080) - Proxied via `api.tenzro.xyz` (also serves `/faucet`)
- MCP (3001) - Proxied via `mcp.tenzro.xyz`
- A2A (3002) - Proxied via `a2a.tenzro.xyz`
- Solana MCP (3003) - Proxied via `solana-mcp.tenzro.xyz`
- Ethereum MCP (3004) - Proxied via `ethereum-mcp.tenzro.xyz`
- Canton MCP (3005) - Proxied via `canton-mcp.tenzro.xyz`
- LayerZero MCP (3006) - Proxied via `layerzero-mcp.tenzro.xyz`
- Chainlink MCP (3007) - Proxied via `chainlink-mcp.tenzro.xyz`
- Li.Fi MCP (3008) - Proxied via `lifi-mcp.tenzro.xyz`
...
3. Caddy terminates TLS and routes by hostname:
   - `rpc.tenzro.xyz` -> `tenzro-rpc-internal:8545` (JSON-RPC)
   - `api.tenzro.xyz` -> `tenzro-rpc-internal:8080` (Web API + `/faucet`)
   - `mcp.tenzro.xyz` -> `tenzro-rpc-internal:3001` (Tenzro MCP)
   - `a2a.tenzro.xyz` -> `tenzro-rpc-internal:3002` (A2A protocol)
   - `solana-mcp.tenzro.xyz` -> `tenzro-rpc-internal:3003`
   - `ethereum-mcp.tenzro.xyz` -> `tenzro-rpc-internal:3004`
   - `canton-mcp.tenzro.xyz` -> `tenzro-rpc-internal:3005`
   - `layerzero-mcp.tenzro.xyz` -> `tenzro-rpc-internal:3006`
   - `chainlink-mcp.tenzro.xyz` -> `tenzro-rpc-internal:3007`
   - `lifi-mcp.tenzro.xyz` -> `tenzro-rpc-internal:3008`
...
- **Terraform State:** GCS bucket `tenzro-operator-project-terraform-state`
...
# Set project
gcloud config set project tenzro-operator-project
...
The Terraform state is stored in GCS: `gs://tenzro-operator-project-terraform-state/testnet/`

If the bucket doesn't exist, create it:

```bash
gsutil mb -p tenzro-operator-project -l us-central1 gs://tenzro-operator-project-terraform-state
gsutil versioning set on gs://tenzro-operator-project-terraform-state
```
...
# - registry_url: us-central1-docker.pkg.dev/tenzro-operator-project/tenzro
...
gcloud builds submit \
  --tag us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:$TAG \
  --project=tenzro-operator-project \
...
# rpc.tenzro.xyz -> 35.224.150.186
# api.tenzro.xyz -> 35.224.150.186
# mcp.tenzro.xyz -> 35.224.150.186
# a2a.tenzro.xyz -> 35.224.150.186
# solana-mcp.tenzro.xyz    -> 35.224.150.186
# ethereum-mcp.tenzro.xyz  -> 35.224.150.186
# canton-mcp.tenzro.xyz    -> 35.224.150.186
# layerzero-mcp.tenzro.xyz -> 35.224.150.186
# chainlink-mcp.tenzro.xyz -> 35.224.150.186
# lifi-mcp.tenzro.xyz      -> 35.224.150.186
```
...
# Web API health
curl https://api.tenzro.xyz/verify/health

# Status endpoint
curl https://api.tenzro.xyz/status

# Faucet (POST request body) — served on api.tenzro.xyz at /faucet (no /api/ prefix)
curl -X POST https://api.tenzro.xyz/faucet \
  -H "Content-Type: application/json" \
  -d '{"address":"0x..."}'

# MCP protocol
curl -X POST https://mcp.tenzro.xyz/mcp \
  -H "Content-Type: application/json" \
...
```
Type  Name                              Value
A     rpc.tenzro.xyz                35.224.150.186
A     api.tenzro.xyz                35.224.150.186
A     mcp.tenzro.xyz                35.224.150.186
A     a2a.tenzro.xyz                35.224.150.186
A     solana-mcp.tenzro.xyz         35.224.150.186
A     ethereum-mcp.tenzro.xyz       35.224.150.186
A     canton-mcp.tenzro.xyz         35.224.150.186
A     layerzero-mcp.tenzro.xyz      35.224.150.186
A     chainlink-mcp.tenzro.xyz      35.224.150.186
A     lifi-mcp.tenzro.xyz           35.224.150.186
```
...
gcloud compute forwarding-rules list --project=tenzro-operator-project

# Check GCP quotas
gcloud compute project-info describe --project=tenzro-operator-project
```
...
gcloud projects get-iam-policy tenzro-operator-project
```
...
kubectl set image statefulset/tenzro-validator \
  -n tenzro-testnet \
  tenzro-node=us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:$TAG
...
gsutil cp ./validator-0-backup-*.tar.gz \
  gs://tenzro-operator-project-backups/validators/
```
...
gsutil cp gs://tenzro-operator-project-backups/validators/validator-0-backup-20260319.tar.gz \
  ./backup.tar.gz
        ...
        spec:
          ...
          - name: backup
            ...
            - |
              ...
              gsutil cp - gs://tenzro-operator-project-backups/validators/backup-$(date +%Y%m%d-%H%M).tar.gz
...
1. **Configure DNS:** Point `*.tenzro.xyz` domains to LoadBalancer IP (35.224.150.186)
...
gsutil rm -r gs://tenzro-operator-project-terraform-state
```
