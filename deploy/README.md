...
gcloud builds submit \
  --tag us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:latest \
  --project=tenzro-operator-project \
  --timeout=1800s \
...
docker build \
  -t us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:latest \
  -f Dockerfile \
  .

# Push to registry
docker push us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:latest
```
...
kubectl set image statefulset/tenzro-validator \
  -n tenzro-testnet \
  tenzro-node=us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:v0.2.0
  ...
  deploy:
    ...
    steps:
      ...
      - name: Build and push image
        run: |
          gcloud builds submit \
            --tag us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:${{ github.sha }} \
            --project=tenzro-operator-project \
            --timeout=1800s \
            --machine-type=e2-highcpu-32 \
            .
      ...
      - name: Deploy to GKE
        run: |
          ...
          kubectl set image statefulset/tenzro-validator \
            -n tenzro-testnet \
            tenzro-node=us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:${{ github.sha }}
...
steps:
  - name: 'gcr.io/cloud-builders/docker'
    args:
      - 'build'
      - '-t'
      - 'us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:$COMMIT_SHA'
      - '-t'
      - 'us-central1-docker.pkg.dev/tenzro-operator-project/tenzro/tenzro-node:latest'
      - '.'
    timeout: 1800s
